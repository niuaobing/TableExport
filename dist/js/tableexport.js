    ...
    $.fn.tableExport = function (options) {
        ...
        this.each(function () {
            var $el = $(this),
                ...
                fileName = settings.fileName === "id" ? ($el.attr('id') ? $el.attr('id') : $.fn.tableExport.defaultFileName) : $el.attr('fileName'),
                exporters = {
            ...
            function checkCaption(exportButton) {
				...
				$caption.length ? $caption.append(exportButton) : $el.prepend('<caption id="expbtn" class="' + bootstrapSpacing + settings.position + '">' + exportButton + '</caption>');
				//if(confirm('验证成功，继续导出excel文件？')){
					f_click();
				//}
            ...
            function createObjButton(dataObject, myContent, myClass) {
                var exportButton = "<button data-fileblob='" + dataObject + "' style=\"display:none;\" class='" + bootstrapClass + bootstrapTheme + myClass + "'>" + myContent + "</button>";
		...
		function f_click(){//alert(3333);
			var object = $("button[data-fileblob]").data("fileblob"),
			    data = object.data,
			    fileName = object.fileName,
			    mimeType = object.mimeType,
			    fileExtension = object.fileExtension;
